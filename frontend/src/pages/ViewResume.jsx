...
function normalizeResumeData(resume) {
  ...
  // ดึง template จากทุก field ที่เป็นไปได้ + console.log เพื่อ debug
  const rawTpl = (resume.template || resume.templateName || "").toLowerCase().trim();
  const template = ["classic", "modern", "professional"].includes(rawTpl) ? rawTpl : "classic";

  console.log("[ViewResume] raw backend keys:", Object.keys(resume));
  console.log("[ViewResume] template field:", JSON.stringify(resume.template), "→ normalized:", template);
...
/* ─────────────────────────────────────────────
   HELPERS — copy ตรงจาก Resume.jsx
───────────────────────────────────────────── */
    ...
    <div style={{ display: "flex", gap: "14px", marginBottom: isLast ? 0 : "18px" }}>
      ...
      <div style={{ flex: 1, paddingBottom: isLast ? 0 : "4px" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: "2px" }}>
          ...
          {period && <span style={{ fontSize: "11px", color: color, fontWeight: "600", whiteSpace: "nowrap", marginLeft: "8px" }}>{period}</span>}
        </div>
...
/* ─────────────────────────────────────────────
   SKILLS DISPLAY — copy ตรงจาก Resume.jsx
───────────────────────────────────────────── */
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: "10px" }}>
        {skills.filter(s => s.type === "Hard Skill").map((s, i) => (
          <div key={s.id || i}>
            ...
            <div style={{ height: "4px", backgroundColor: sidebar ? "rgba(255,255,255,0.15)" : "#e8e8e8", borderRadius: "3px", overflow: "hidden" }}>
...
/* ─────────────────────────────────────────────
   RESUME CONTENT — 1:1 copy จาก Resume.jsx
   (ตัด highlightField prop ออก)
───────────────────────────────────────────── */
function ResumeContent({ data }) {
  const themeColor = data.themeColor || "#d4af37";
  const tpl = ["classic", "modern", "professional"].includes(data.template) ? data.template : "classic";
    ...
    return (
      ...
      >
        ...
        <div style={{ backgroundColor: tc, padding: "36px 44px 30px", display: "flex", alignItems: "center", gap: "26px", position: "relative" }}>
          {data.image ? (
            <div style={{ width: "96px", height: "96px", borderRadius: "50%", border: "3px solid rgba(255,255,255,0.85)", overflow: "hidden", flexShrink: 0 }}>
              <img src={data.image} alt="Profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            </div>
          ) : (
            <div style={{ width: "96px", height: "96px", borderRadius: "50%", border: "3px solid rgba(255,255,255,0.4)", backgroundColor: "rgba(0,0,0,0.15)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
    ...
    return (
      ...
      >
        ...
        <div style={{ display: "flex", alignItems: "center", gap: "28px", padding: "36px 44px 28px", borderBottom: `3px solid ${tc}` }}>
          {data.image ? (
            <div style={{ width: "110px", height: "110px", borderRadius: "50%", border: `3px solid ${tc}`, overflow: "hidden", flexShrink: 0 }}>
              <img src={data.image} alt="Profile" style={{ width: "100%", height: "100%", objectFit: "cover" }} />
            </div>
          ) : (
            <div style={{ width: "110px", height: "110px", borderRadius: "50%", border: `3px solid ${tc}`, backgroundColor: "#f5f5f5", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
          ...
          <div style={{ flex: 1 }}>
            ...
            <p style={{ margin: "0 0 6px", fontSize: "16px", fontWeight: "600", color: tc }}>
              {data.title || ""}
            </p>
  ...
  return (
    ...
    >
      {/* HEADER */}
      <div style={{ backgroundColor: themeColor, padding: "36px 40px 28px", display: "flex", alignItems: "center", gap: "28px" }}>
        {data.image ? (
          ...
          <div style={{ width: "110px", height: "110px", borderRadius: "50%", border: "4px solid rgba(255,255,255,0.4)", backgroundColor: "rgba(0,0,0,0.15)", flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center" }}>
            <svg width="46" height="46" viewBox="0 0 42 42" fill="none">
              <circle cx="21" cy="16" r="9" fill="rgba(255,255,255,0.45)" />
              <path d="M4 38c0-9.39 7.61-17 17-17s17 7.61 17 17" fill="rgba(255,255,255,0.45)" />
            </svg>
    ...
    const fetchResumeData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          // รองรับ response แบบ { resume: {...} } หรือ flat { id, title, ... }
          const raw = data.resume || data.data || data;
          console.log("[ViewResume] full raw from backend:", JSON.stringify(raw, null, 2));
